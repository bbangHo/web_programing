...
exports.userMatching = function (app, db) {
  app.put('/user/partner', (req, res) => {
    ...
    try {
      ...
      // 매칭할 수 있는 인원이 너무 적으면 이상형에 가까운 파트너를 찾는 로직을 무시하고 랜덤으로 매칭합니다.
      if (partnerList.length < 5) {
        randomMatching();
      } else {
        personalityMatching();
      }
    ...
    function randomMatching() {
      let index = Math.floor((Math.random() * 100) % partnerList.length);
      console.log(partnerList[index]);
      console.log(index);
    ...
    function personalityMatching() {
      ...
      if (user.partnerIdx == "") {
        if (user.personality != "" && user.idealPersonality != "") {
          ...
          let tmp = list.filter(list => (list.similarity >= 7));
          console.log(tmp);
          let rand = Math.floor((Math.random() * 100) % tmp.length);
